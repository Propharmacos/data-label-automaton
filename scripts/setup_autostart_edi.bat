@echo off
title ProPharmacos - Setup Autostart (PC da Edi)
color 0B

echo.
echo ============================================================
echo  ProPharmacos - Setup Autostart - PC DA EDI
...
REM Criar atalho na pasta Startup do usuario (metodo mais confiavel que schtasks)
set STARTUP=%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup
echo [1/3] Criando atalho na pasta Startup...

REM Remover atalho antigo se existir
del "%STARTUP%\ProPharmacos_Agente.bat" >nul 2>&1
del "%STARTUP%\ProPharmacos_Agente.lnk" >nul 2>&1

REM Criar .bat wrapper na Startup (mais confiavel que .lnk via cmd)
(
    echo @echo off
    echo cd /d "%PASTA%"
...
REM Tambem registrar no Task Scheduler como backup
...
echo.
echo ============================================================
echo  CONFIGURACAO CONCLUIDA - PC DA EDI
echo.
echo  Pasta:    %PASTA%
