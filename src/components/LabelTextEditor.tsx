...
const tiposPrescritores: Record<string, { conselho: string }> = {
  ...
  '4': { conselho: 'CRNE' }, '5': { conselho: 'CRP' }, '6': { conselho: 'CRF' },
          ...
          <div className="flex items-center gap-1 border border-border rounded-md px-1">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (!isNaN(val)) setPrintQuantity(Math.max(1, Math.min(300, val)));
            ...
            />
            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => setPrintQuantity(q => Math.min(300, q + 1))}>
              <Plus className="h-3 w-3" />
            </Button>
          </div>
