...
def buscar_ph_componente(cursor, cdpro_comp, cdfil):
    ...
    """
    try:
        # Tentativa 1: com CDFIL específico
        cursor.execute("""
        ...
        if row and row[0]:
            ...
            if ph_val and ph_val != '0':
                return ph_val
        # Tentativa 2: sem filtro de CDFIL (produto pode estar em outra filial)
        cursor.execute("""
            SELECT FIRST 1 PH FROM FC06100
            WHERE CDPRO = ? AND PH IS NOT NULL AND PH <> '0'
            ORDER BY DTVAL DESC
        """, (int(cdpro_comp),))
        row = cursor.fetchone()
        if row and row[0]:
            ph_val = str(row[0]).strip()
            if ph_val and ph_val != '0':
                print(f"    [PH_COMP] pH encontrado sem filtro CDFIL para {cdpro_comp}: {ph_val}")
                return ph_val
    except Exception as e:
...
def buscar_requisicao(nr_requisicao):
    ...
    try:
        ...
        def buscar_componentes_kit_fc12111(cursor, nrrqu, serier, cdfil):
            ...
            try:
                ...
                for row in rows:
                    ...
                    # Busca pH do componente via função centralizada (com fallback sem CDFIL)
                    ph_comp = buscar_ph_componente(cursor, cdpro_comp, cdfil_int)
        ...
        def buscar_componentes_kit_fc05100(cursor, cdfrm, cdfil):
            ...
            try:
                ...
                for row in rows:
                    ...
                    # Busca pH via função centralizada (com fallback sem CDFIL)
                    ph = buscar_ph_componente(cursor, cdsac, cdfil)
        ...
        for idx, item in enumerate(itens):
            ...
            # Remove sufixos de via de administração do nome da fórmula
            for _sfx in [' ENDOVENOSO', ' ENDOVENOSA', ' ENDOVEN', ' ENDOVE', ' ENDOV']:
                if nome_formula.upper().endswith(_sfx):
                    nome_formula = nome_formula[:len(nome_formula)-len(_sfx)].strip()
                    break
...
# =====================================================
GITHUB_RAW_URL_SERVIDOR = "https://raw.githubusercontent.com/marketingpropharmacos/data-label-automaton/main/servidor.py"
SERVIDOR_FILE = os.path.abspath(__file__)
UPDATE_INTERVAL_SERVIDOR = 120  # verifica a cada 2 minutos


def _hash_arquivo_srv(conteudo: str) -> str:
    return hashlib.md5(conteudo.encode("utf-8")).hexdigest()


def _codigo_atual_srv() -> str:
    try:
...
def _aplicar_update_servidor(novo_codigo: str) -> bool:
    try:
        atual = _codigo_atual_srv()
        backup_path = SERVIDOR_FILE + ".bak"
        with open(backup_path, "w", encoding="utf-8") as f:
            f.write(atual)
        with open(SERVIDOR_FILE, "w", encoding="utf-8") as f:
            f.write(novo_codigo)
        print(f"[UPDATE_SRV] Arquivo atualizado. Backup em {backup_path}")
        return True
    except Exception as e:
        print(f"[UPDATE_SRV] Erro ao salvar: {e}")
        return False


def _reiniciar_servidor():
    """Reinicia o processo do servidor após 3 segundos."""
    def _do():
        time.sleep(3)
        print("[UPDATE_SRV] Reiniciando servidor...")
        try:
            subprocess.Popen([sys.executable] + sys.argv, close_fds=True)
        except Exception as e:
            print(f"[UPDATE_SRV] Erro ao reiniciar: {e}")
        finally:
            os._exit(0)
    threading.Thread(target=_do, daemon=True).start()


def _loop_auto_update_servidor():
    """Thread que verifica o GitHub a cada UPDATE_INTERVAL_SERVIDOR segundos."""
    print(f"[UPDATE_SRV] Auto-update ativo — verificando a cada {UPDATE_INTERVAL_SERVIDOR}s")
    while True:
        time.sleep(UPDATE_INTERVAL_SERVIDOR)
        try:
            novo = _fetch_latest_servidor()
            if not novo:
                continue
            atual = _codigo_atual_srv()
            if _hash_arquivo_srv(novo) != _hash_arquivo_srv(atual):
                print("[UPDATE_SRV] Nova versão detectada no GitHub! Aplicando...")
                if _aplicar_update_servidor(novo):
                    _reiniciar_servidor()
                    break
            else:
                print("[UPDATE_SRV] Sem alterações no GitHub.")
        except Exception as e:
            print(f"[UPDATE_SRV] Erro no ciclo: {e}")


@app.route('/api/update', methods=['POST'])
def update_servidor():
    """Força update imediato do servidor.py a partir do GitHub."""
    print("[UPDATE_SRV] Update manual solicitado...")
    novo = _fetch_latest_servidor()
    if not novo:
        return jsonify({"success": False, "message": "Não foi possível baixar do GitHub"}), 502
    atual = _codigo_atual_srv()
    if _hash_arquivo_srv(novo) == _hash_arquivo_srv(atual):
        return jsonify({"success": True, "message": "Já está na versão mais recente", "updated": False})
    if not _aplicar_update_servidor(novo):
...
if __name__ == '__main__':
    ...
    print(f"Auto-update: verificando GitHub a cada {UPDATE_INTERVAL_SERVIDOR}s")
    print("=" * 50)

    # Inicia thread de auto-update em background
    t = threading.Thread(target=_loop_auto_update_servidor, daemon=True)
    t.start()
